    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(
                    attrs,
                    R.styleable.AutofitText,
                    defStyle,
                    0);
            sizeToFit = ta.getBoolean(R.styleable.AutofitText_sizeToFit, sizeToFit);
            minTextSize = ta.getDimensionPixelSize(R.styleable.AutofitText_minTextSize,
                    minTextSize);
            precision = ta.getFloat(R.styleable.AutofitText_precision, precision);
            ta.recycle();
        }
